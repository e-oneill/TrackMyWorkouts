...
class App extends React.Component{
  ...
  async handleSignUp(name, email, password, weight = 0, height = 0)
  {
    ...
    await createUserWithEmailAndPassword(this.context.auth, email, password).then((userCredential) => {
      user = userCredential.user;
      this.context.user = user;
      this.context.authCredential = userCredential;
      if (!weight) {weight = 0}
  ...
  async updateUserDetails(name, weight, height) {
    await setDoc(doc(this.context.database, "users", this.context.user.uid), {
      ...
      this.setState({user:{uuid: this.state.user.uuid, userEmail: this.state.user.userEmail, name: name, weight: weight, height: height}})
